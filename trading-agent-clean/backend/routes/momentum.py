...
async def get_momentum_tv_confirmed(
    ...
    db = get_database()
    saved_rows_count = await db.momentum_tv_confirmations.count_documents(query)
    cursor = db.momentum_tv_confirmations.find(query, {"_id": 0}).sort("updated_at", -1)
    if limit is not None:
        cursor = cursor.limit(limit)
    rows = [row async for row in cursor]
    response = {"index_name": clean_index, "limit": limit, "saved_rows_count": saved_rows_count, "rows_count": len(rows), "rows": rows}
    if checked_timeframes:
