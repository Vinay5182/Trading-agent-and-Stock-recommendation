...
from tv_confirmation import confirm_swing_symbol_timeframes, enforce_tv_confirmation_safety, latest_source_candle_at
...
def stamp_swing_confirmation_row(row: dict, confirmed_at: str | None = None) -> dict:
    timestamp = confirmed_at or utc_now_iso()
    row["confirmed_at"] = timestamp
    row["swing_confirmed_at"] = timestamp
    row["updated_at"] = timestamp
    row["source_candle_at"] = row.get("source_candle_at") or latest_source_candle_at(row)
    return row


def serialize_swing_confirmation_row(row: dict) -> dict:
    serialized = dict(row)
...
def build_swing_identity(row: dict, now: str = None) -> tuple[dict, dict]:
    base_identity = {
...
async def save_confirmation_row(row: dict) -> dict[str, Any]:
    ...
    identity, base_identity = build_swing_identity(row, now)
...
async def run_swing_tv_confirmation(
    ...
    for candidate in candidates:
        ...
        if save:
            try:
                identity, _ = build_swing_identity(row, row.get("updated_at") or row.get("swing_confirmed_at") or row.get("confirmed_at"))
                final_row = await get_database().swing_tv_confirmations.find_one(identity)
                if final_row:
    ...
    if save:
        try:
            ...
            selected_cnt, confirmed_cnt, rejected_cnt = categorize_tv_rows("swing", rows)
